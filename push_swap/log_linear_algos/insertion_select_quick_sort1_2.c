...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   insertion_select_quick_sort1_2.c                   :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: aazzaoui <aazzaoui@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/20 13:12:25 by aazzaoui          #+#    #+#             */
/*   Updated: 2025/02/22 14:13:54 by aazzaoui         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	index_place7(int item)
{
	...
	{
		if (!is_min_and_max7(nums[i], nums[i + 1]))
		{
	...
	return (is_between_0_and_count7(item));
}
...
int	best_a_move7(t_ins_sel_move *move)
{
	...
	move->index_a = index_place7(move->num_b);
	if (move->index_a == -1)
		return (100);
	move->num_a = ((int *)(stack_a()->contate))[move->index_a];
	cost = normal_cost7(*move);
	rr_cost = rr_cost7(*move);
	rrr_cost = rrr_cost7(*move);
	if (rr_cost <= rrr_cost && rr_cost < cost)
		return (set_move7(rr_cost, en_rr, move));
	if (rrr_cost < rr_cost && rrr_cost < cost)
		return (set_move7(rrr_cost, en_rrr, move));
	return (set_move7(cost, en_sa, move));
}
